...
#define XML_PAIR(key, value) 	"<Pair key=\"" key "\"><>" value "</></Pair>"
...
//
	...
	myxml.size = sprintf(myxml.str, "%s"
						"<View id=\"%s%s\">"
						"<Attributes>%s", XML_HEADER, scanning_roms ? "seg_wm_rom_" : "seg_mygames", scanning_roms ? roms_path[roms_index] : "", proxy_include);
